...
Protocol.prototype.sslhandshake = function(cb) {
  return this._handshakeSequence = this._enqueue(new Sequences.SSLHandshake(this._config, cb));
};
...
Protocol.prototype._emitPacket = function(packet) {
  ...
  // Checking for TLS/SSL packet request
  // If we are requesting a secure connection, we need to upgrade our stream to TLS1
  if(packet.enableTLS)
  {
    var stream = this;
    ...
    require('../StartTLS')(this._connection._socket, this._config.ssl, function() {
...
Protocol.prototype._determinePacket = function(sequence, ssl) {
  var firstByte = this._parser.peak();

  if (sequence.determinePacket) {
    var Packet = sequence.determinePacket(firstByte, this._parser, ssl);
    if (Packet) {
      return Packet;
    }
  }
